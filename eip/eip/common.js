...
function Com_GetDate(date, type, format) {
	...
	year = parseInt(date.substr(index, 4));
	// 取月份，必须2位
	index = format.indexOf("MM");
	month = parseInt(date.substr(index, 2));
	// 取日期，必须2位
	index = format.indexOf("dd");
	day = parseInt(date.substr(index, 2));
	...
	if ('datetime' == type) {
		...
		hour = parseInt(date.substr(index, 2));
		// 取分钟，必须2位
		index = format.indexOf("mm");
		minute = parseInt(date.substr(index, 2));
		// 取秒，必须2位。如无数据则取0
		index = format.indexOf("ss");
		second = (index > -1) ? parseInt(date.substr(index, 2)) : 0;
		// 取毫秒，必须3位。如无数据则取0
		index = format.indexOf("SSS");
		millisecond = (index > -1) ? parseInt(date.substr(index, 3)) : 0;
