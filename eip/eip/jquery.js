 ...
 */
    ...
    bI.extend({
        ...
        acceptData: function(b3) {
            ...
            return !e || e !== true && (b3.getAttribute && b3.getAttribute("classid") === e)
        }
    });
    bI.fn.extend({
        data: function(b5, b8) {
    ...
    bI.fn.offset = function(b3) {
        ...
        b7 = bo(b6);
        return {
            top: b5.top + (b7.pageYOffset || e.scrollTop || b6.body.scrollTop) - (e.clientTop || 0),
            left: b5.left + (b7.pageXOffset || e.scrollLeft || b6.body.scrollLeft) - (e.clientLeft || 0)
        }
    }
    ;
    bI.offset = {
    ...
    if (typeof module === "object" && module && typeof module.exports === "object") {
        ...
        if (typeof define === "function" && typeof seajs == "object") {
            define("resource/js/jquery", function(b3, e, b4) {
                b4.exports = a1.LUI.$ = bI
            })
        }
    }
    bI.ajaxSetup({
        beforeSend: function(b4, b3) {
            var e = b3.url;
            if (e.indexOf("?") > -1) {
                e = e + "&s_ajax=true"
            } else {
        ...
        complete: function(b3, e) {
            if (a1.LUI) {
                LUI.ajaxComplete(b3)
            }
        }
    });
    if (a1.Com_RegisterFile) {
...
(function(a) {
    if (typeof define === "function" && define.amd) {
        define("jQuery-ajaxTransport-XDomainRequest", ["jquery"], a)
    } else {
        a(jQuery)
    }
}(function(b) {
    ...
    b.ajaxTransport("* text html xml json", function(f, h, g) {
        if (!f.crossDomain || !f.async || !d.test(f.type) || !c.test(f.url) || !a.test(f.url)) {
            return
        }
        var e = null;
        return {
            send: function(l, j) {
                ...
                ;
                e.onload = function() {
                    var r = "Content-Length: " + e.responseText.length + "\r\nContent-Type: " + e.contentType;
                    var m = {
                        code: 200,
                        message: "success"
                    };
                    ...
                    try {
                        if (k === "html" || /text\/html/i.test(e.contentType)) {
                            o.html = e.responseText
                        } else {
                            if (k === "json" || (k !== "text" && /\/json/i.test(e.contentType))) {
                                try {
                                    o.json = b.parseJSON(e.responseText)
                                } catch (q) {
                                    m.code = 500;
                                    m.message = "parseerror"
                                }
                            } else {
                                if (k === "xml" || (k !== "text" && /\/xml/i.test(e.contentType))) {
                                    var p = new ActiveXObject("Microsoft.XMLDOM");
                                    p.async = false;
                                    try {
                                        p.loadXML(e.responseText)
                                    } catch (q) {
                                        p = undefined
                                    }
                                    if (!p || !p.documentElement || p.getElementsByTagName("parsererror").length) {
                                        m.code = 500;
                                        m.message = "parseerror";
                ...
                ;
                if (h.data) {
                    i = (b.type(h.data) === "string") ? h.data : b.param(h.data)
                }
                e.open(f.type, f.url);
                e.send(i)
            },
