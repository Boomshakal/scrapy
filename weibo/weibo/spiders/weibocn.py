...
class WeibocnSpider(Spider):
    ...
    def start_requests(self):
        yield Request(url=self.fans_url.format(containerid=self.fans_head+self.start_id,since_id=self.since_id),callback=self.parse_fans)
        yield Request(url=self.followers_url.format(containerid=self.followers_head+self.start_id,page=self.page),callback=self.parse_followers)
    ...
    def parse_user(self, response):
        ...
        yield Request(url=self.fans_url.format(containerid=self.fans_head+id,since_id=self.since_id),callback=self.parse_fans)
        yield Request(url=self.followers_url.format(containerid=self.followers_head+id,page=self.page),callback=self.parse_followers)
